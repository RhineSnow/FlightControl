...
{  
	...
	RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOB|RCC_AHB1Periph_GPIOD, ENABLE);//使能GPIOB,G时钟
  ...
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_100MHz;//100MHz
  GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_UP;//上拉
  GPIO_Init(GPIOD, &GPIO_InitStructure);//初始化PD1,2
  ...
  GPIO_SetBits(GPIOB,GPIO_Pin_14);//PB14输出1,防止SPI FLASH干扰NRF的通信 
 	...
 	SPI1_Init();    		//初始化SPI1,可认为是通用SPI设置，之后会在NRF24L01_SPI_Init中专门针对nrf设置SPI，没有多余SPI设备时两者可结合
